...
@interface AsyncImageDownloader : NSObject <NSURLConnectionDelegate>
{
    ...
    void (^successCallback)(UIImage *image,NSDictionary *data,NSDictionary *dataTemp);
    void (^failCallback)(NSError *error);
}

@property NSString *mediaURL;
@property NSString *fileURL;
@property NSDictionary *name;
@property NSDictionary *dataTemp;

-(id)initWithMediaURL:(NSString *)theMediaURL data:(NSDictionary *)data tempData:(NSDictionary *) dataTemp successBlock:(void (^)(UIImage *image,NSDictionary *data,NSDictionary *dataTemp))success failBlock:(void(^)(NSError *error))fail;
