...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self initialize];
        self.backgroundColor = [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.40f];
    }
    return self;
}
...
- (void)setProgress:(CGFloat)progress {
    ...
    self.animationTimer = [NSTimer scheduledTimerWithTimeInterval:0.00 target:self selector:@selector(incrementAnimatingProgress) userInfo:nil repeats:YES];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    rect = CGRectMake(5,self.frame.size.height-20,self.frame.size.width-12, 15);
...
- (void)drawProgressBackground:(CGContextRef)context inRect:(CGRect)rect {
    ...
    CGContextSetFillColorWithColor(context, [UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.30f].CGColor);
    [roundedRect fill];
...
- (void)drawProgress:(CGContextRef)context withFrame:(CGRect)frame {
    CGRect rectToDrawIn = CGRectMake(5,self.frame.size.height-20, frame.size.width * self.progress, frame.size.height);
...
//    if (rect.size.width > 40) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(6,self.frame.size.height-20,self.frame.size.width-12, rect.size.height)];
    
        label.adjustsFontSizeToFitWidth = YES;
        label.backgroundColor = [UIColor clearColor];
        label.textAlignment = NSTextAlignmentCenter;
        label.text = [NSString stringWithFormat:@"%@%.2f%%",self.tip,self.progress*100];
        label.font = [UIFont fontWithName:@"MicrosoftYaHei" size:11.0f];
     //   UIColor *baseLabelColor = [self.color isLighterColor] ? [UIColor blackColor] : [UIColor whiteColor];
      ...
      //  label.textColor = [baseLabelColor colorWithAlphaComponent:0.6];
        ...
        [label drawTextInRect:CGRectMake(6,self.frame.size.height-20,self.frame.size.width-12, rect.size.height)];
 //   }
}
